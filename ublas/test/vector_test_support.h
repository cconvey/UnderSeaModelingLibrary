...
#define USML_VECTOR_REAL_TESTER(R, V, F) \
	usml::ublas::vector_test::vector_real_helper(R, V, F)

#define USML_VECTOR_COMPLEX_TESTER(R, V, F) \
	usml::ublas::vector_test::vector_complex_helper(R, V, F)

#define USML_VECTOR_CR_TESTER(R, V, F) \
	usml::ublas::vector_test::vector_cr_helper(R, V, F)

#define USML_VECTOR_POW_TESTER(R, T1, T2) \
	usml::ublas::vector_test::vector_power_helper(R, T1, T2)

#define USML_VECTOR_POW_SCALAR1_TESTER(R, S, V) \
	usml::ublas::vector_test::vector_power_scalar1_helper(R, S, V)

#define USML_VECTOR_POW_SCALAR2_TESTER(R, V, S) \
	usml::ublas::vector_test::vector_power_scalar2_helper(R, V, S)
...
void vector_real_helper(const vector<double>& result, const vector<double>& v,
                        Functor f) {
 ...
 */
template <class Functor>
void vector_complex_helper(const vector<complex<double> >& result,
 ...
 */
template <class Functor>
void vector_cr_helper(const vector<complex<double> >& result,
...
void vector_power_helper(const R& result, const V1& v1, const V2& v2) {
...
void vector_power_scalar1_helper(const R& result, const S& s, const V& v) {
  typedef typename R::value_type result_type;
  for (size_t i = 0; i < result.size(); ++i) {
...
void vector_power_scalar2_helper(const R& result, const V& v, const S& s) {
  typedef typename R::value_type result_type;
  for (size_t i = 0; i < result.size(); ++i) {
