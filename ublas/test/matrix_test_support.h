...
#define USML_MATRIX_REAL_TESTER(R, M, F) \
	usml::ublas::matrix_test::matrix_real_helper(R, M, F)

#define USML_MATRIX_COMPLEX_TESTER(R, M, F) \
	usml::ublas::matrix_test::matrix_complex_helper(R, M, F)

#define USML_MATRIX_CR_TESTER(R, M, F) \
	usml::ublas::matrix_test::matrix_cr_helper(R, M, F)

#define USML_MATRIX_POW_TESTER(R, T1, T2) \
	usml::ublas::matrix_test::matrix_power_helper(R, T1, T2)

#define USML_MATRIX_POW_SCALAR1_TESTER(R, S, M) \
	usml::ublas::matrix_test::matrix_power_scalar1_helper(R, S, M)

#define USML_MATRIX_POW_SCALAR2_TESTER(R, M, S) \
	usml::ublas::matrix_test::matrix_power_scalar2_helper(R, M, S)
...
void matrix_real_helper(const matrix<double>& R, const matrix<double>& M,
                        Functor f) {
 ...
 */
template <class Functor>
void matrix_complex_helper(const matrix<complex<double> >& result,
...
template <class Functor>
void matrix_cr_helper(const matrix<complex<double> >& result,
...
void matrix_power_helper(const R& result, const M1& m1, const M2& m2) {
...
void matrix_power_scalar1_helper(const R& result, const S& s, const M& m) {
  typedef typename R::value_type result_type;
  for (size_t i = 0; i < result.size1(); ++i) {
    for (size_t j = 0; j < result.size2(); ++j) {
...
void matrix_power_scalar2_helper(const R& result, const M& m, const S& s) {
  typedef typename R::value_type result_type;
  for (size_t i = 0; i < result.size1(); ++i) {
    for (size_t j = 0; j < result.size2(); ++j) {
