 ...
 */
wposition1::wposition1(const wposition1& location, double range,
                       double bearing) {
    ...
    double lat2 =
        asin(sin(lat1) * cos(range) + cos(lat1) * sin(range) * cos(bearing));
    double lng2 = lng1 + atan2(sin(bearing) * sin(range) * cos(lat1),
 ...
 */
wposition1::wposition1(const wvector1& other) : wvector1(other) {}
...
double wposition1::gc_range(const wposition1& location, double* bearing) const {
    ...
    double hav1at = sin(0.5 * (lat1 - lat2));
    hav1at *= hav1at;
    double havlng = sin(0.5 * (lng1 - lng2));
    havlng *= havlng;
    ...
    if (bearing != nullptr) {
        if (abs(range) < 1e-6 || abs(lat1) < 1e-10) {
            ...
            *bearing = atan2(sin(lng1 - lng2) * cos(lat2),
                             cos(lat1) * sin(lat2) -
                                 sin(lat1) * cos(lat2) * cos(lng1 - lng2));
            *bearing = fmod(TWO_PI - *bearing, TWO_PI);
        }
    }
