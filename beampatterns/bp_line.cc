 ...
 * of the array,
 */
void bp_line::beam_level(const bvector& arrival, const seq_vector::csptr& frequencies,
        vector<double>* level, const bvector& steering, double sound_speed) const {
    ...
    *level = abs2(element_div(
            sin(_num_elements * (*level)) + 1e-200,
    ...
    for (double n = 1.0; n <= _num_elements; ++n) {
        for (double m = 1.0; m <= _num_elements; ++m) {
            if (n == m) {
                ...
                *level += element_div(
                        element_prod(cos(anm * str), sin(anm)), anm);
            }
        }
    }
