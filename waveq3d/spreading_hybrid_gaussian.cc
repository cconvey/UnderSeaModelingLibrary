...
const vector<double>& spreading_hybrid_gaussian::intensity(
    ...
    for (size_t f = 0; f < _wave._frequencies->size(); ++f) {
        _spread(f) =
            SPREADING_WIDTH * sound_speed(0, 0) / (*_wave._frequencies)(f);
    }
...
void spreading_hybrid_gaussian::intensity_de(size_t de, size_t az,
    ...
    cout << "spreading_hybrid_gaussian::intensity_de:" << endl
         << "\ttime=" << _wave._time << " de(" << de
         << ")=" << (*_wave._source_de)(de) << " az(" << az
         << ")=" << (*_wave._source_az)(az)
         << " srf=" << _wave._curr->surface(de, az)
         << " btm=" << _wave._curr->bottom(de, az)
         << " cst=" << _wave._curr->caustic(de, az) << " offset=" << offset(1)
         << " distance=" << distance(1) << endl;
#endif
    ...
    cout << "\t** center" << endl
         ...
         << " cell_dist=" << cell_dist << " cell_width=" << cell_width
         << " beam_width=" << sqrt(_beam_width) << " norm=" << _norm_de(d)
         << " intensity=" << _intensity_de << endl
         << "\t** lower " << endl;
#endif
    ...
    for (d = (int)de - 2; d >= 0; --d) {
        bool virtual_ray =
            _wave._curr->on_edge(d + 1, az) && _wave._curr->on_edge(d, az);
        ...
        cout << "\tde(" << d << ")=" << (*_wave._source_de)(d)
             ...
             << " intensity=" << _intensity_de << endl;
#endif
        if (_intensity_de(0) / old_tl < THRESHOLD) {
            break;
        }
        if (virtual_ray) {
            break;
...
void spreading_hybrid_gaussian::intensity_az(size_t de, size_t az,
    ...
    // Check for an abnormal normalization constant, ie when DE is close to a de
    // branch pt

    if (de >= max_de) {
        _new_norm = _norm_az(1, a);
    } else {
        _new_norm = _norm_az(de, a);
    }
    ...
    while ((a % max_az) != az_lower) {
        ...
        // Check for an abnormal normalization constant, ie when DE is close to
        // a de branch pt
        ...
        if (_intensity_az(0) / old_tl < THRESHOLD) {
            break;
        }
        if (a == 0) {
            a += max_az - 1;
 ...
 */
double spreading_hybrid_gaussian::width_de(size_t de, size_t az,
                                           const vector<double>& offset) {
    double L1;
    double L2;
    double length1;
    ...
    if (v < 1e-10) {
        ...
        L2 = wvector1(pos1, de, az_wrap)
                 .distance(wvector1(pos1, de + 1, az_wrap));
    ...
    const wposition& pos2 =
        (offset(0) < 0.0) ? _wave._prev->position : _wave._next->position;
    L1 = wvector1(pos2, de, az).distance(wvector1(pos2, de + 1, az));
    if (v < 1e-10) {
        length2 = L1;
    } else {
        L2 = wvector1(pos2, de, az_wrap + 1)
                 .distance(wvector1(pos2, de + 1, az_wrap + 1));
 ...
 */
double spreading_hybrid_gaussian::width_az(size_t de, size_t az,
                                           const vector<double>& offset) {
    double L1;
    double L2;
    double length1;
    ...
    if (v < 1e-10 || abs(v - 1.0) < 1e-10) {
        ...
        L2 = wvector1(pos1, de_upper + 1, az)
                 .distance(wvector1(pos1, de_upper + 1, az_wrap));
        length1 = (1.0 - v) * L1 + v * L2;
    }
    ...
    if (v < 1e-10 || abs(v - 1.0) < 1e-10) {
        ...
        L2 = wvector1(pos2, de_upper + 1, az)
                 .distance(wvector1(pos2, de_upper + 1, az_wrap));
        length2 = (1.0 - v) * L1 + v * L2;
    }
