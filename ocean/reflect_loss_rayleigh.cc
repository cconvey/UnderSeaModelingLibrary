...
struct reflect_loss_rayleigh::bottom_type_table
    reflect_loss_rayleigh::lookup[] = {
        ...
        //    ---------   ---  ----  ---  -----  ----
        {bottom_type_enum::clay, 1.5, 1.00, 0.2, 0.00, 1.0},
        {bottom_type_enum::silt, 1.7, 1.05, 1.0, 0.00, 1.5},
        {bottom_type_enum::sand, 1.9, 1.10, 0.8, 0.00, 2.5},
        {bottom_type_enum::gravel, 2.0, 1.20, 0.6, 0.00, 1.5},
        {bottom_type_enum::moraine, 2.1, 1.30, 0.4, 0.40, 1.0},
        {bottom_type_enum::chalk, 2.2, 1.60, 0.2, 0.67, 0.5},
        {bottom_type_enum::limestone, 2.4, 2.00, 0.1, 1.00, 0.2},
        {bottom_type_enum::basalt, 2.7, 3.50, 0.1, 1.67, 0.2},
};
 ...
 * Initialize model with impedance mis-match factors.
 */
reflect_loss_rayleigh::reflect_loss_rayleigh(double density, double speed,
                                             double att_bottom,
                                             double speed_shear,
                                             double att_shear)
    : _density_water(1000.0),
...
void reflect_loss_rayleigh::reflect_loss(const wposition1& /*location*/,
    ...
    complex<double> cosAp;
    complex<double> cosAs;
    complex<double> Zb = impedance(_density_bottom, _speed_bottom, _att_bottom,
                                   angle, &cosAp, false);

    // compute shear wave reflection components

    if (_speed_shear != 0.0 || _att_shear != 0.0) {
        const complex<double> Zs = impedance(_density_bottom, _speed_shear,
                                             _att_shear, angle, &cosAs, true);
    ...
    noalias(*amplitude) =
        -20.0 * log10(abs(R)) * scalar_vector<double>(frequencies->size(), 1.0);
    if (phase != nullptr) {
        noalias(*phase) =
            arg(R) * scalar_vector<double>(frequencies->size(), 1.0);
    }
}
 ...
 */
complex<double> reflect_loss_rayleigh::impedance(double density, double speed,
                                                 double attenuation,
                                                 double angle,
                                                 complex<double>* cosA,
                                                 bool shear) const {
